...
create or replace procedure find_functions(out functions_count integer) as $$
declare
  function_name text;
  function_param text;
  user_oid oid;
...
create or replace procedure delete_sql_dml_triggers(out deleted_triggers integer) as $$
declare
  trig_name text;
  table_name text;
begin
...
create or replace procedure find_sql_objects(ref refcursor, containe text) as $$
begin
  open ref for (
